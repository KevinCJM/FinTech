    ...
    if (response && response.words && response.words.length > 0) {
      ...
      function displayWord(word) {
        contentDiv.innerHTML = `
          <strong>${word.word}</strong> ${word.phonetic ? `/${word.phonetic}/` : ""}
          ${word.audio ? `<button id="reviewPlayAudio" style="margin-left: 10px; cursor: pointer;">🔊</button>` : ""}
          <br>
          中文意思：${word.translation.word}<br>
          <hr>
          <em>例句：</em> ${word.sentence}<br>
          <em>翻译：</em> ${word.translation.sentence}
        `;

        if (word.audio) {
          const playButton = document.getElementById("reviewPlayAudio");
